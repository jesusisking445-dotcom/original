...
// Accepts either a plain "HH:MM" string or an { startTime, endTime } object
// and always returns the { startTime, endTime } shape the schema expects.
...
// ── BOOK APPOINTMENT (works for guests too — requires contact details if not logged in) ──
router.post('/', optional, rateLimiter.appointment, asyncHandler(async (req, res) => {
  const {
    clinicId, dentistId, referralId,
    appointmentDate, timeSlot, appointmentTime, // accept either key name
    serviceType, notes,
  ...
  if (!clinicId || !appointmentDate || !rawTimeSlot) {
    throw new AppError('clinicId, appointmentDate and a time slot are required', 400);
  }
  if (!req.user && (!patientName || !patientPhone || !patientEmail)) {
    throw new AppError('Name, phone and email are required when booking without an account', 400);
  }

  const clinic = await Clinic.findById(clinicId).catch(() => null);
  if (!clinic || clinic.status !== 'active') {
    throw new AppError('Selected clinic was not found. Please choose a clinic from the list again.', 404);
  }
  ...
  try {
    await emailService.sendAppointmentConfirmation(appointment, clinic, req.user || { email: patientEmail, firstName: patientName });
  } catch (error) {
    // Booking still succeeds even if the confirmation email fails to send
  }
...
// ── TAKEN TIME SLOTS for a clinic on a given date (public — powers the booking calendar) ──
router.get('/taken-slots', asyncHandler(async (req, res) => {
  const { clinicId, date } = req.query;
  if (!clinicId || !date) {
    throw new AppError('clinicId and date are required', 400);
  }
  ...
  const taken = appointments.map(a => a.timeSlot && a.timeSlot.startTime).filter(Boolean);
...
router.get('/:id', protect, asyncHandler(async (req, res) => {
  ...
  if (!isOwner && !isAssignedDentist && !isStaff) {
...
router.patch('/:id/cancel', protect, asyncHandler(async (req, res) => {
  ...
  if (!isOwner && !['admin', 'clinic_manager'].includes(req.user.role)) {
    throw new AppError('Unauthorized', 403);
  }

  if (!['pending', 'confirmed'].includes(appointment.status)) {
    throw new AppError('Cannot cancel an appointment in its current status', 400);
  }
...
// ── STATUS UPDATE (dentist / clinic staff) ────────────────
...
router.patch('/:id/rate', protect, asyncHandler(async (req, res) => {
  ...
  if (!appointment.user || appointment.user.toString() !== req.user._id.toString()) {
    throw new AppError('Unauthorized', 403);
  }
